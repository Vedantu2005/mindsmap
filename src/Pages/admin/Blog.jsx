...
  );
  ...
  // NEW: Heading Dropdown Component
  const handleHeadingChange = (e) => {
    const value = e.target.value;
    if (value) {
      // formatBlock applies a block tag (like H1, P, DIV) to the current line
      execCommand('formatBlock', `<${value}>`); 
    }
    // Optional: Reset dropdown value after applying command 
    e.target.value = ''; 
  };
  ...
  const HeadingDropdown = () => (
    <select
      ...
      className="p-1 h-8 text-slate-600 border border-slate-300 rounded hover:border-orange-500 transition-colors cursor-pointer text-sm bg-white"
      defaultValue=""
    >
      <option value="" disabled hidden>Heading Size</option>
      <option value="p">Paragraph (Default)</option>
...
                        <div className="flex flex-wrap items-center p-2 gap-1 border-b border-slate-200 bg-white">
                            {/* NEW: Heading/Font Size Dropdown */}
                            <HeadingDropdown />
                            <Divider />
