...
// Updated BlogCard to receive state
const BlogCard = ({ post }) => (
  <div className="bg-white rounded-lg border border-gray-200/80 shadow-sm hover:shadow-xl transition-all duration-300 overflow-hidden text-left h-full flex flex-col group">
    ...
    <div className="p-6 flex-grow flex flex-col">
      ...
      <h3 className="mt-2 text-xl font-bold text-gray-900 flex-grow leading-snug">
        <Link to="/blogdetailspage" state={{ id: post.id }} className="hover:text-[#00B4D8] transition-colors">
          {post.title}
        </Link>
      </h3>
      ...
      <Link to="/blogdetailspage" state={{ id: post.id }} className="mt-4 text-[#00B4D8] font-semibold hover:text-cyan-700 self-start inline-flex items-center">
        Read More
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10">
          ...
          {!loading && blogs.map((post) => (
            <BlogCard key={post.id} post={{
              id: post.id, // Passed explicitly for state
              date: post.date || '',
